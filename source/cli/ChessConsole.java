...
import source.puzzles.Puzzle;
import source.puzzles.PuzzleDatabase;
...
public class ChessConsole {
    ...
    private Chess game;
    private final PuzzleDatabase database;
        ...
        for (int i = Chess.ROW - 1; i >= 0; i--) {
            for (int j = 0; j < Chess.COLUMN; j++) {
                if (shouldHighlight(i, j, reachablePositions)) {
                    if (game.getPieceAt(new Position(i, j)) == null){
                        System.out.print(RED + "." + RESET);
                        continue;
                    }
                    System.out.print(RED + game.getPieceAt(new Position(i, j)) + RESET);
                }
                else if(i == position.getY() && j == position.getX()){
                    System.out.print(Blue + game.getPieceAt(new Position(i, j)) + RESET);
                }
                else {
                    if (game.getPieceAt(new Position(i, j)) == null){
                        System.out.print("  ");
                    ...
                    System.out.print(game.getPieceAt(new Position(i, j)));
                }
            }
            System.out.println();
        }
